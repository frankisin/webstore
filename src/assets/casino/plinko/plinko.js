...
const leftPanelWidth = 0; // Width of the left panel (in pixels)
...
const biasStrength = 0.00009; // Bias force toward center
...
const multiplierColors = {
    ...
    5: "#FFA41F",   // Deep yellow, less orange
    2: "#FFC300",   // Vivid yellow
    0.2: "#FFEB00", // Bright pure yellow
};
...
function createScoreCards() {
    ...
    for (let i = 0; i < bottomRow.length - 1; i++) {
        ...
        // Assign the multiplier from the multipliers array
        const multiplier = multipliers[i % multipliers.length]; // Ensure values loop if fewer cards
        ...
        scoreCards.push({
            ...
            lastHit: 0, // Timestamp of the last interaction
            cooldown: 500 // Cooldown duration in milliseconds
        });
...
function drawScoreCards() {
    ...
    frontGradient.addColorStop(0, "#FD0241");       // Start with red
    frontGradient.addColorStop(0.15, "#FB4627");   // Transition to orange-red
    frontGradient.addColorStop(0.5, "#fee505");    // Bright yellow at the center
    frontGradient.addColorStop(0.85, "#FB4627");   // Back to orange-red
    frontGradient.addColorStop(1, "#FD0241");      // End with red

    // Define gradient for the darker back part
    ...
    backGradient.addColorStop(0, "#A5002F");       // Darker red
    backGradient.addColorStop(0.15, "#A63620");   // Darker orange-red
    backGradient.addColorStop(0.5, "#B88902");    // Darker yellow at the center
    backGradient.addColorStop(0.85, "#A63620");   // Darker orange-red
    backGradient.addColorStop(1, "#A5002F");      // Darker red

    scoreCards.forEach((card, index) => {
        //const yOffset = bounceHeights[index] || 0; // Apply the bounce offset
        const yOffset = card.bounceOffset || 0;

        // Draw the darker "back" part of the card
        ctx.fillStyle = backGradient;
        ctx.beginPath();
        const radius = card.height / 6; // Rounded corners
        const backOffset = 5; // Offset for the back part
        ctx.moveTo(card.x - card.width / 2 + radius, card.y + yOffset + backOffset);
        ctx.lineTo(card.x + card.width / 2 - radius, card.y + yOffset + backOffset);
        ctx.quadraticCurveTo(
            card.x + card.width / 2, 
            card.y + yOffset + backOffset, 
            card.x + card.width / 2, 
            card.y + radius + yOffset + backOffset
        );
        ctx.lineTo(card.x + card.width / 2, card.y + card.height - radius + yOffset + backOffset);
        ctx.quadraticCurveTo(
            card.x + card.width / 2, 
            card.y + card.height + yOffset + backOffset, 
            card.x + card.width / 2 - radius, 
            card.y + card.height + yOffset + backOffset
        );
        ctx.lineTo(card.x - card.width / 2 + radius, card.y + card.height + yOffset + backOffset);
        ctx.quadraticCurveTo(
            card.x - card.width / 2, 
            card.y + card.height + yOffset + backOffset, 
            card.x - card.width / 2, 
            card.y + card.height - radius + yOffset + backOffset
        );
        ctx.lineTo(card.x - card.width / 2, card.y + radius + yOffset + backOffset);
        ctx.quadraticCurveTo(
            card.x - card.width / 2, 
            card.y + yOffset + backOffset, 
            card.x - card.width / 2 + radius, 
        ...
        // Draw the lighter "front" part of the card
        ...
        ctx.fillStyle = "black";
        const displayText = (index < 2 || index >= scoreCards.length - 2)
            ? `${card.multiplier}`
            : `${card.multiplier}x`;
...
// Check if a ball lands in a score card
function checkBallInScoreCards(ball) {
    for (let i = 0; i < scoreCards.length; i++) {
        ...
        if (
            ...
            // Update multiplier tracker
            if (multiplierTracker[card.multiplier] !== undefined) {
                multiplierTracker[card.multiplier] += 1; // Track hits for this multiplier
            }

            // Add multiplier to the displayed list
            displayedMultipliers.push({
                multiplier: card.multiplier,
                color: multiplierColors[card.multiplier] || "#FFFFFF", // Use card color
                timestamp: Date.now(), // Track time for display duration
                x: card.x,
                y: card.y - 20, // Position slightly above the card
            });
    ...
    for (let row = 0; pinsInRow <= maxPins; row++) {
        ...
        for (let col = 0; col < pinsInRow; col++) {
            ...
            pins.push({ 
                x, 
                y,
                lastHit: 0, // Timestamp of last interaction
                cooldown: 300 // Cooldown duration in milliseconds
            });
        }
    ...
    for (let i = balls.length - 1; i >= 0; i--) {
        ...
        for (const pin of pins) {
            ...
            if (distance < ballRadius + pinRadius) {
                ...
                // Check if animation cooldown has passed
                const now = Date.now();
                if (now - pin.lastHit > pin.cooldown) {
                    pin.lastHit = now; // Update last hit timestamp
                    ripples.push({ x: pin.x, y: pin.y, radius: 0, opacity: 1.0 }); // Add ripple
                }
            }
        }
...
function drawStatistics() {
    const infoX = 300; // Left padding for stats
...
function updateHtmlStats() {
    document.getElementById('totalBalls').textContent = `Total Balls: ${totalBallsSpawned}`;
    const multiplierStats = document.getElementById('multiplierStats');
    multiplierStats.innerHTML = ''; // Clear existing stats
    ...
    for (const [multiplier, count] of Object.entries(multiplierTracker)) {
        ...
        li.textContent = `${multiplier}x: ${count} (${percentage}%)`;
        multiplierStats.appendChild(li);
    }
}
...
function spawnBalls(number) {
    ...
    const maxX = rightPanelStart + rightPanelWidth * 0.53; // End slightly to the left of center
...
function handlePlayButtonClick(event) {
    ...
    const mouseX = event.clientX - rect.left; // Adjust mouse x relative to canvas
    const mouseY = event.clientY - rect.top; // Adjust mouse y relative to canvas
    ...
    if (
        ...
        // Spawn one ball at the center of the right panel
        const centerX = leftPanelWidth + (width - leftPanelWidth) / 2;
        balls.push(createBall(centerX, -ballRadius));
        clickSound.currentTime = 0;
        clickSound.play();
        totalBallsSpawned++; // Update total balls counter
    }
}
let playButton;
...
function gameLoop() {
    ...
    // Draw statistics in the left panel
    drawStatistics();

    // Draw right panel background
    ctx.fillStyle = "#0F212D"; // Main game background
    ctx.fillRect(leftPanelWidth, 0, width - leftPanelWidth, height);
